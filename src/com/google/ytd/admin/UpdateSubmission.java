...
/**
 * Servlet responsible for updating submissions, both in the AppEngine datastore
 * and on YouTube.
 */
@Singleton
public class UpdateSubmission extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		try {
			...
			// We can only update moderation for videos that were uploaded with our
			// developer key.
			if (currentStatus != newStatus && entry.getVideoSource() == VideoSource.NEW_UPLOAD
				...
				adminApiManager
						.updateModeration(entry.getVideoId(), newStatus == ModerationStatus.APPROVED);
			}
	 ...
	 */
	private boolean removeFromPlaylist(YouTubeApiHelper apiManager, VideoSubmission videoSubmission) {
		long assignmentId = videoSubmission.getAssignmentId();
		Assignment assignment = assignmentDao.getAssignmentById(assignmentId);
